 ...
 */
async function runSharpOperation(operation: string, ...args: string[]): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    childProcess.on("close", (code) => {
      if (code === 0) {
        ...
        reject(new Error(`Sharp operation failed with code ${code}: ${stderrData}`));
      }
    });
  });
}
...
async function generateGradientIcon(
  ...
  const shadowGradient = ctx.createLinearGradient(0, shadowY - shadowHeight, 0, shadowY);
  ...
  try {
    ...
    console.error(`Error creating temp directory: ${error instanceof Error ? error.message : String(error)}`);
  }
  ...
  if (svgPath) {
    try {
      ...
      console.error(`Error processing SVG: ${error instanceof Error ? error.message : String(error)}`);
...
function parseArgs(): { startColor: string; endColor: string; svgPath?: string | null; outputPath?: string; text?: string | null } {
  const args = process.argv.slice(2);

  if (args.length < 2) {
    console.error("Error: Please provide start and end colors as hex values");
    console.error("Usage: bun icon.ts #startColor #endColor [svgPath] [outputPath] [text]");
    process.exit(1);
  }
  ...
  if (!hexRegex.test(startColor) || !hexRegex.test(endColor)) {
    console.error("Error: Colors must be valid hex values (e.g., #FF5500 or #F50)");
    process.exit(1);
  }
  ...
  return { startColor, endColor, svgPath: svgPath || undefined, outputPath, text };
}
...
(async () => {
  try {
    ...
    await generateGradientIcon(startColor, endColor, svgPath, outputPath, text === undefined ? null : text);
  } catch (error) {
